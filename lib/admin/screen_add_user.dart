...
class Fragment1State extends State<ScreenAddUser> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
          ...
          FlatButton(
            ...
            onPressed: (){
               ...
               if(firstname.isEmpty||lastname.isEmpty||position.isEmpty||username.isEmpty||password.isEmpty){
                 ...
                 _showAlertDialog('กรุณาเลือกสาขาวิชาก่อน!');
               }else {
                 sendData(
                     titlename,
                     firstname,
        ...
        title: Text('เพิ่มผู้ใช้งาน',style: textStyleAppbar),
        centerTitle: true,
      ),
      body: Center(
      ...
      bottomNavigationBar: branchID==null?makeBottom:null,
    );
  }
  void _toggleLogin() {
    setState(() {
  ...
  Future<http.Response> callAddUser(title, firstname,lastname,position,username,password,usertype,brnachID) async {
    final paramDic = {
      "TitleName": title,
      "FirstName": firstname,
      "LastName": lastname,
    ...
    final addUser = await http.post(
      Server().IPAddress + "/AddUser.php", // change with your API
      body: paramDic,
    );
    return addUser;
  }

  void sendData(title, firstname,lastname,position,username,password,usertype,brnachID,mContext)async{
    showDialog(
        context: context,
        builder: (BuildContext context) {
    ...
    await onLoadAction(
        title, firstname,lastname,position,username,password,usertype,brnachID,mContext
    );
    Navigator.pop(context);
    Navigator.pop(context);
  }
  Future<bool> onLoadAction(
    ...
    await callAddUser(title, firstname, lastname, position, username, password, usertype,brnachID);
    return true;
  }
}